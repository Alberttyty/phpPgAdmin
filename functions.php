...
function doEdit($msg = '')
{
    ...
    if ($fndata->recordCount() > 0) {
        ...
        htmlspecialchars((string)$_POST['formComment']), "</textarea></td></tr>\n";
        ...
        if ($data->hasFunctionAlterOwner()) {
            ...
            while (!$users->EOF) {
                ...
                echo "<option value=\"", htmlspecialchars($uname), "\"", ($uname == $_POST['formFuncOwn']) ? ' selected="selected"' : '', ">", htmlspecialchars($uname), "</option>\n";
                $users->moveNext();
            }
            echo "</select>\n";
